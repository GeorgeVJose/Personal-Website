...
def get_projects():
    selected_repos= ["100-Prisoners-Problem", "Covid-Vaccine-Alert", "DRISHTE-Public",
                    "Vadakel-Electricity-Dashboard", "Gmail_Notifier",
     ]
    api_url = f'https://api.github.com/users/georgevjose/repos'
    cards_list = requests.get(api_url).json()
    cards_list_selected = {x["name"]: x for x in cards_list if x["name"] in selected_repos}

    cards_list_selected["100-Prisoners-Problem"]["site"] = "https://georgevjose-100-prisoners-problem.streamlitapp.com/"
    cards_list_selected["Vadakel-Electricity-Dashboard"]["site"] = "https://vadakel-electricity.streamlitapp.com/"

    return cards_list_selected.values()
...
@app.route('/')
def main_page():
    return render_template('index.html', title='George Jose - Homepage')
